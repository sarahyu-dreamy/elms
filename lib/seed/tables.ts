...
export const TABLE_SPECS: TableSpec[] = [
  ...
  },
  {
    ...
    note: 'student_sub 를 필수로 두지 마세요. 학생이 처음 로그인해야 sub 가 생기므로, 명단은 이름으로 먼저 넣고 나중에 연결합니다.',
    columns: [
      { name: 'class_id', type: 'uuid', required: true },
      { name: 'student_name', type: 'text', required: true, note: '명단의 이름' },
      { name: 'student_sub', type: 'text', note: '로그인 후 연결. 처음엔 비어 있습니다' },
      { name: 'cohort', type: 'text' },
  ...
  },
  {
    ...
    label: '차시',
    group: 'classwork',
    neededFor: '대면/온라인 구분, 학생 진도표',
    note: '단원 하나에 4차시. 단원마다 대면/온라인 비중을 다르게 잡습니다.',
    compositeUnique: ['unit_id', 'order_index'],
    columns: [
      { name: 'unit_id', type: 'uuid', required: true },
      { name: 'order_index', type: 'int4', required: true, note: '1~4' },
      { name: 'mode', type: 'text', required: true, default: 'onsite', note: 'onsite / online' },
      { name: 'title', type: 'text' },
      { name: 'note', type: 'text', note: '교사용 메모' },
    ],
  },
  {
    name: 'activities',
    label: '학습 활동',
    group: 'classwork',
    neededFor: '온라인 차시 학습, 과제',
    note: '과제 테이블을 따로 두지 않습니다. 마감일과 배점을 여기 두면 과제가 됩니다.',
    columns: [
      { name: 'session_id', type: 'uuid', required: true },
      { name: 'class_id', type: 'uuid', note: '비우면 모든 반 공통' },
      { name: 'activity_type', type: 'text', required: true, default: 'vocab_drill', note: 'vocab_drill/text_read/quiz/speaking/writing' },
      { name: 'title', type: 'text', required: true },
      { name: 'instructions', type: 'text' },
      { name: 'target_id', type: 'uuid', note: '지문·문항 등 대상' },
      { name: 'is_required', type: 'bool', default: 'true', note: '★ 다음 단원 진입 조건' },
      { name: 'due_at', type: 'timestamptz', note: '비우면 마감 없음' },
      { name: 'max_score', type: 'int4' },
      { name: 'order_index', type: 'int4', default: '0' },
      { name: 'is_published', type: 'bool', default: 'false' },
      { name: 'created_by', type: 'text' },
    ],
  },
  {
    ...
    label: '학생별 활동 진행',
    group: 'progress',
    neededFor: '진도표, 다음 단원 진입 판정',
    compositeUnique: ['student_sub', 'activity_id'],
    columns: [
      { name: 'student_sub', type: 'text', required: true },
      { name: 'activity_id', type: 'uuid', required: true },
      { name: 'status', type: 'text', default: 'not_started', note: 'not_started/in_progress/completed' },
      { name: 'score', type: 'numeric', note: '자동 채점 결과' },
      { name: 'attempts', type: 'int4', default: '0' },
  ...
  },
  {
    ...
    label: '제출·피드백',
    group: 'classwork',
    neededFor: '녹음·작문 제출과 채점',
    compositeUnique: ['activity_id', 'student_sub'],
    columns: [
      { name: 'activity_id', type: 'uuid', required: true },
      { name: 'student_sub', type: 'text', required: true },
      ...
      { name: 'pronunciation_score', type: 'numeric', note: '발음 API 점수. 학생도 봅니다' },
    ],
  },
